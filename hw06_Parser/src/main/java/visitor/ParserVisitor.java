...
public class ParserVisitor implements TokenVisitor {
    ...
    public void visit(Brace brace) {
        if (brace.getTokenType() == TokenType.OPEN_BRACE) {
            stack.push(brace);
        } else {
            while (!stack.isEmpty() && stack.peek().getTokenType() != TokenType.OPEN_BRACE) {
    ...
    public void visit(Operation operation) {
        while (!stack.isEmpty() && operation.getPriority() <= operationPriority(stack.peek())) {
            polish.add(stack.pop());
        }
    ...
    private int operationPriority(Token token) {
        if (token instanceof Operation) {
            return ((Operation) token).getPriority();
        }
        return 0;
    }
